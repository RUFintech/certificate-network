...
composer network install -a certificate-network@0.1.35.bna -c PeerAdmin@hlfv1

composer network start --networkName certificate-network --networkVersion 0.1.35 --networkAdmin admin --networkAdminEnrollSecret adminpw --card PeerAdmin@hlfv1
...
composer card import -f ./cards/sudodean@certificate-network.card
...
composer card delete --card tumih@certificate-network
composer identity issue -c admin@certificate-network -f ./cards/tumih@certificate-network.card -u tumih -a "resource:org.university.certification.Observer#13"